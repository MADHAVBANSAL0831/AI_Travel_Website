...
/**
 * Streaming version of runAgent for low-latency voice mode
 * Yields chunks as they're generated while still supporting tool calls
 */
export async function* runAgentStream(
  userMessage: string,
  history: Array<{ role: string; content: string }> = []
): AsyncGenerator<{ type: 'chunk' | 'searchResults' | 'done'; content?: string; searchResults?: any[] }> {
  try {
    console.log("\n========================================");
    console.log("LANGCHAIN AGENT STREAM");
    console.log("========================================");
    ...
    try {
      personalityContext = await getPersonalityContext(userMessage);
      console.log("Personality Context:", personalityContext.slice(0, 100) + "...");
    } catch (e) {
      console.log("No personality context available:", e);
    }
...
When presenting search results, format them nicely with emojis.`;
    ...
    // First LLM call (non-streaming to detect tool calls)
    const response = await llmWithTools.invoke(messages);
    ...
    // Check if there are tool calls
    if (response.tool_calls && response.tool_calls.length > 0) {
      console.log("Tool calls detected:", response.tool_calls.map(tc => tc.name));
      ...
      if (searchResults.length > 0) {
        yield { type: 'searchResults', searchResults };
      }

      // Prepare final response with streaming
      const flightToolCall = toolCalls.find(tc => tc.name === "search_flights");
      const hotelToolCall = toolCalls.find(tc => tc.name === "search_hotels");
      const hasFlights = flightToolCall?.result?.found === true;
      const hasHotels = hotelToolCall?.result?.found === true;
...
Be helpful and offer to search again with different parameters.`));
    ...
    yield { type: 'chunk', content: "I apologize, but I encountered an error. Please try again or rephrase your request." };
    yield { type: 'done' };
  }
}
